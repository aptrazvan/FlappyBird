...
(define (position-abilities abilities)
	(map (lambda (x) (if (null? (get-ability-pos x)) (struct-copy ability x [pos (random-position)]) x)) abilities))
...
(define (compose-abilities L)
	(apply compose (map (lambda (x) ((get-ability-next x))) L)))
...
(define (fill-abilities initial n abilities)
	(let iter ((x initial))
          (if (> n (length x))
              (iter (cons (choice-abilities abilities) x)) x)))
