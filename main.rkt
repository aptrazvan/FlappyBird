...
(define-struct bird-values(y v-y))
...
(define-struct state (bird-values pipes variables abilities score))
...
(define (get-initial-state)
  (state (bird-values bird-initial-y 0) (add-first-pipe) (variables initial-gravity initial-momentum initial-scroll-speed) (abilities '() '()) 0))
...
(define (get-bird state)
  (state-bird-values state))
(define (get-bird-y bird)
  (bird-values-y bird))

(define (get-state-variables state)
  (state-variables state))
(define (get-gravity state)
  (variables-gravity (get-state-variables state)))
(define (get-momentum state)
  (variables-momentum (get-state-variables state)))
(define (get-scroll-speed state)
  (variables-scroll-speed (get-state-variables state)))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-values bird [v-y (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ") (struct-copy state current-state [bird-values (next-state-bird-onspace (get-bird current-state) (get-momentum current-state))])
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (pipe1) (struct-copy pipe pipe1 [x (- (get-pipe-x pipe1) scroll-speed)])) pipes))
...
(define (clean-pipes pipes)
  (if (< (+ (get-pipe-x (car pipes)) pipe-width) 0) (cdr pipes)
      pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes) (append pipes (list (pipe (+ pipe-width pipe-gap (get-pipe-x (car (reverse pipes)))) (+ added-number (random random-threshold)))))
      pipes))
...
(define (check-pipe-collisions bird pipes)
  (foldr (lambda (x y) (or x y)) #f (map (lambda (pipe) (if (or (check-collision-rectangles (make-posn (get-pipe-x pipe) 0) (make-posn (+ (get-pipe-x pipe) pipe-width) (get-pipe-gap-y pipe))
                                                                (make-posn bird-x (get-bird-y bird)) (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height)))
                                     (check-collision-rectangles (make-posn (get-pipe-x pipe) (+ (get-pipe-gap-y pipe) pipe-self-gap)) (make-posn (+ (get-pipe-x pipe) pipe-width) scene-height)
                                                                (make-posn bird-x (get-bird-y bird)) (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height)))) #t #f)) pipes)))
...
(define (next-state state1)
  (struct-copy state state1 [bird-values (next-state-bird (get-bird state1) (get-gravity state1))]
               [pipes (next-state-pipes (get-pipes state1) (get-scroll-speed state1))]
               [score (+ (get-score state1) 0.1)]))
...
(define (upper-pipe gap) (rectangle pipe-width gap "solid" "green"))
(define (lower-pipe gap) (rectangle pipe-width (- pipe-height gap pipe-self-gap) "solid" "green"))

(define (place-pipe-image pipes image) (place-image (upper-pipe (get-pipe-gap-y (car pipes)))
                                                        (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                                                        (quotient (get-pipe-gap-y (car pipes)) 2)
                                           (place-image (lower-pipe (get-pipe-gap-y (car pipes)))
                                                        (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                                                        (+ (+ (get-pipe-gap-y (car pipes)) pipe-self-gap)
                                                           (quotient (- pipe-height (+ (get-pipe-gap-y (car pipes)) pipe-self-gap)) 2))
                                                        ((lambda () (if (not (empty? (cdr pipes))) (place-pipe-image (cdr pipes) image) image))))))

(define (draw-frame state)
  (match-let ([(bird-values bird-y v-y) (get-bird state)])
    (place-image bird (+ bird-x (quotient bird-width 2)) (+ bird-y (quotient bird-height 2))
                 (place-image ground (quotient scene-width 2) (+ ground-y (quotient ground-height 2))
                              (place-image (score-to-image (get-score state)) text-x text-y
                                           (place-pipe-image (get-pipes state) initial-scene))))))
...
(define fast-ability (ability (hourglass "tomato") 10 null (lambda (scroll-speed) ((+ scroll-speed 1)))))
...
(define slow-ability (ability (hourglass "mediumseagreen") 30 null (lambda (scroll-speed) ((if (> (- scroll-speed 1) 5) (- scroll-speed 1) 5))))) 
...
(define (clean-abilities pipes)
	((if (< (posn-x (get-ability-pos (car pipes))) 0) (cdr pipes) pipes)))


; Muta abilităţile vizibile spre stanga.
; echivalent cu move-pipes.
(define (move-abilities abilities scroll-speed)
	(map (lambda (ability1) (struct-copy ability ability1 [pos (make-posn (- (posn-x (get-ability-pos ability1)) scroll-speed)
                                                                              (posn-y (get-ability-pos ability1)))])) abilities))
